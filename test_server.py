...
def _fake_mcp_context(working_dir: str) -> MagicMock:
    """Minimal FastMCP context so list_roots() yields a workspace path in unit tests."""
    root = MagicMock()
    root.uri.path = working_dir
    roots_result = MagicMock(roots=[root])
    mock_session = MagicMock()
    mock_session.list_roots = AsyncMock(return_value=roots_result)
    mock_ctx = MagicMock()
...
class TestAnalyzeFileChanges:
    ...
    async def test_returns_json_string(self):
        ...
        with patch.object(mcp, "get_context", return_value=_fake_mcp_context("/tmp/fake-repo")):
